...
namespace TheaterService.Controllers
{
	...
	public class ShowTimeController(AppDbContext db) : ControllerBase
	{
		...
		public IActionResult GetShowTime([FromRoute] int hall_id, [FromRoute] int id)
		{
			...
			if (showtime == null || showtime.Hall_Id != hall_id)
				return NotFound();
			return Ok(showtime);
		}

		[HttpPost]
		public async Task<IActionResult> CreateNewShowTime(
			[FromRoute] int hall_id,
			[FromBody] NewShowTime newShowtime
		)
		{
			...
			if (movie.Duration + 30 >= showtimeDuration)
				return BadRequest();

			// Check for overlap shows in the newShow duration
			var overlaps = await db
				.Showtimes.Where(s =>
					s.Hall_Id == hall_id
					&& s.Date == newShowtime.Date
					&& (
						(
							newShowtime.Start_time >= s.Start_time
							&& newShowtime.Start_time < s.End_time
						)
						|| (
							newShowtime.End_time > s.Start_time
							&& newShowtime.End_time <= s.End_time
						)
						|| (
							newShowtime.Start_time <= s.Start_time
							&& newShowtime.End_time >= s.End_time
			...
			if (overlaps)
				return Conflict(
					"Another showtime is already scheduled in this hall during that period."
				);

			var showtime = db.Showtimes.Add(
				new Showtime
				{
					Id = 0,
			...
			return CreatedAtAction(
				nameof(GetShowTime),
				new { id = showtime.Entity.Id, hall_id = hall_id },
				showtime.Entity
			);
		}
